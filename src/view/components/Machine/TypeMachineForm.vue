<template>
  <v-form ref="formType" lazy-validation @submit.prevent="doSubmit">
    <div>
      <p class="mb-0">Type Machine Code <span class="error--text">*</span></p>
      <v-text-field
        v-model="form.code"
    ...
    <div>
      <p class="mb-0">Type Machine Name <span class="error--text">*</span></p>
      <v-text-field
        v-model="form.name"
        :rules="[(v) => !!v || 'This field is required']"
        outlined
    ...
    async doSubmit() {
      if (this.$refs.formType.validate()) {
        ...
        if (this.selectedType?.id) {
          response = await this.$store.dispatch(UPDATE_TYPES, {
            id: this.selectedType.id,
            body: this.form,
          });
        } else {
          response = await this.$store.dispatch(SUBMIT_TYPES, this.form);
        }
